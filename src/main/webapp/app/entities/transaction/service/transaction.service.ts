...
export class TransactionService {
  ...
  protected convertDateFromClient(transaction: ITransaction): ITransaction {
    return Object.assign({}, transaction, {
      createdOn: transaction.createdOn?.isValid() ? transaction.createdOn.toJSON() : undefined,
      updatedOn: transaction.updatedOn?.isValid() ? transaction.updatedOn.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((transaction: ITransaction) => {
        transaction.createdOn = transaction.createdOn ? dayjs(transaction.createdOn) : undefined;
        transaction.updatedOn = transaction.updatedOn ? dayjs(transaction.updatedOn) : undefined;
      });
    }
    return res;
  }
